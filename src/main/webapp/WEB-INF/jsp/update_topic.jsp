        ...
        <div class="panel-body">
            <div class="form-group">
                ...
                <input type="text" class="form-control" id="title" name="title"
                       value="${topic.title}" required="required">
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                    ...
                    <div class="col-sm-10" style="width: 40%">
                        <select class="form-control" id="tab" name="tab">
                            <c:forEach items="${tabs}" var="tab">
                                <option <c:if test="${tab.id==topic.tabId}">selected</c:if> value="${tab.id}">${tab.tabName}</option>
                            </c:forEach>
                        </select>
                    </div>
                </div>
                ...
                <div class="row">
                    <div class="col-sm-2">
                        <button type="button" class="btn btn-default btn-sm" id="submitBut">修改</button>
                    </div>
                </div>
            </div>
        </div>
    ...
    function sendMessage() {
        ...
        $.post(url, params, function (result) {
            ...
            if(result.state == 2){ //发表成功
                alert(result.message);
            //window.history.back();
                window.location = "${pageContext.request.contextPath}/t/${topic.id}";
            }
